package challenges;

public class NumberFive {

    /**
     * Two strings, word1 and word2, are called anagrams if they contain all the same characters in the same
     * frequencies. For example, the anagrams of CAT are CAT, ACT, TAC, TCA, ATC, and CTA.
     *
     * Example:
     * word 1 - cat
     * word 2 - act
     * true
     *
     * Example:
     * word 1 - cat
     * word 2 - acT
     * true
     *
     * Example:
     * word 1 - cat
     * word 2 - actt
     * false
     *
     * Example:
     * word 1 - cat
     * word 2 - acts
     * false
     *
     * @return true if word1 and word2 are case-insensitive anagrams, false otherwise.
     */
    public boolean isAnagram(String word1, String word2) {
        boolean isamatch = true;
        if(word1.length()==word2.length()){
            String [] firstword = word1.toLowerCase().split("");
            String [] secondword = word2.toLowerCase().split("");
            boolean [] matches = new boolean [word1.length()];
            for (int i = 0; i < firstword.length ; i++){
                for (int j = 0; j < secondword.length; j++){
                    if (firstword[i].equals(secondword[j])){
                        matches[i] = true;
                    }
                }
            }
            for(boolean match : matches){
                if (!(match == true)){
                    isamatch = false;
                }
            }
            return isamatch;
        }
        return false;
    }
}
